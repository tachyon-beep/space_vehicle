...
# The referee lives *in* the vehicle repository now, so the vehicle is this file's own root:
...
def test_the_derived_order_puts_transmit_power_before_the_link_budget(tmp_path):
    ...
    assert result.returncode == 0, result.stderr
    # **The marker is the node's line, not its number.** This read `"32. link"`, and a round that
    # added debt text mentioning `link` made the substring test true while the number had moved to
    # 35 — so the split produced the whole report and the assertion below searched the *debts* for a
    # transmit power. A node's position in the order is derived and moves; the line's shape does not.
    import re as _re

    marker = _re.search(r"^\s*\d+\. link\b.*$", result.stdout, _re.M)
    assert marker is not None, result.stdout[-2000:]
    section = result.stdout[marker.start() :]
...
def test_a_derived_value_reads_the_declarations_it_names(tmp_path):
    ...
    converted = {
        ...
        # The radiative balance: the equilibrium temperature the panel relaxes toward, and the
...
def test_an_edge_reading_a_node_that_publishes_no_value_is_a_counted_debt():
    """A node carrying more than one state has no key in the value map, so its edges have no driver.

    `state_values` writes a node key only when one state owns the node — and `plant.stock_flux` says
    the same thing in its own refusal: *"a node that carries two states publishes no value under its
    own name … this is not a value that is late, it is a driver that was never declared."* What no
    check asked was the question at the **source** end. `drains` asks the outbound half for stocks;
    `advances` asks the inbound half which state a flux is integrated into; neither asks whether the
    number the sensitivity multiplies by exists at all.

    Eleven edges are in that position, across five domains, and three of them (`E-CREW-ATM`,
    `E-LM-CREW-ATM`, `E-RAD-THERM`) have targets already blocked for it. **The rest were latent**,
    which is why this is worth a check rather than a repair: an edge that has not yet failed because
    the state it lands on is already blocked upstream is a defect nothing was counting, and the build
    order would have gone on reporting the *target's* blocker until the day the target was fixed and
    the driver turned out to be missing.

    `E-FC-DRAW-O2` is the trap inside the family, and it is why the check tests the *source* rather
    than `advances`: that edge declares `advances: fc_o2_draw_kg_s`, which is the name of a real
    state — on the edge's **target** node. It reads like a driver declaration and is not one, and a
    check written on the field would have passed it.
    """
    result = run_linter(VEHICLE)
    assert result.returncode == 0, result.stdout[-900:]
    # `coupling.yaml:edge` only: the crew domain's own `open_debts` sentence names this family in the
    # same words, and a filter on the phrase alone counts the paragraph explaining the check — which
    # is the trap the round-61 patterns hit from the other side.
    debts = [
        line
        for line in result.stdout.splitlines()
    ...
    assert len(debts) == 11, len(debts)
    for eid in ("E-FC-BUS", "E-FC-HEAT", "E-AMP-LOAD", "E-DYN-GNC", "E-CABIN-CO2-REMOVAL"):
    ...
    from_graph = {
        ...
        if e.source in crowded
        and e.kind != "limit"
        and isinstance(e.sensitivity, dict)
        and e.sensitivity.get("value") not in (None, "UNCONFIGURED")
    }
    # **Two of the thirteen are not named by it, and the reason is not that they are already
    # counted.** `--debts` gives each edge one entry, and `E-CREW-ATM` and `E-LM-CREW-ATM` have
    # theirs spoken for by the target's blocker — the crew's CO2 stocks are refused first, so the
    # worklist reports that. They are named in the family's `open_debts` sentence instead, which is
    # where the count comes from: eleven entries plus two sentences is thirteen.
    named = {
        m.group(1)
        for line in debts
    ...
    assert len(from_graph) == 13, sorted(from_graph)
...
def test_the_build_order_is_derived_and_partitions_the_vehicle():
    ...
    # 134 -> 135 in round 32, with the oxygen supply tank's pressure state: the channel that
    ...
    # 27 -> 28 in round 40: `avionics_bay_heat_w` is the fifth heat rate and the first state on the
    # `internal` sentinel whose rule the corpus *declares* — a `derivation` over three dotted paths
...
def test_the_plant_reports_the_build_order():
    ...
    assert result.returncode == 0, result.stderr
    assert "139 states, by what blocks them" in result.stdout
...
def test_a_declared_rule_is_ready_even_on_the_internal_sentinel(tmp_path):
    """The worklist's one rule about arithmetic was unreachable for every state on the sentinel.

    `build_order` asks "is the rule already in the configuration?" — an `algebraic` state carrying a
    `derivation` over named inputs is *ready*, because the plant evaluates it in a real tick. That
    test sat below the `internal` routing and the routing `continue`s, so for a state on the sentinel
    the answer was always "owes a rule: domain code".

    `avionics_bay_heat_w` is what exposed it: the avionics bay's 360 W, the fifth heat rate, summed
    from the three loads `heat_inputs.csm_avionics_bay` assigns. It is on the sentinel deliberately —
    a heat-rate edge landing on `coldplate_t` would become that lag's canonical driver and relax a
    kelvin state toward a watt — and its rule is a `derivation` the linter re-derives and the plant
    evaluates. The worklist filed it under code that exists, which is the one thing the bucket must
    not do.

    The fixture is the negative: strip the derivation from a copy and the state goes back to owing a
    rule, because then there genuinely is code to write.
    """
    plant = _plant()
    world = plant.load_world(VEHICLE)
    state = next(s for s in world.states if s.id == "avionics_bay_heat_w")
    assert state.node == "internal", state.node
    assert state.method == "algebraic"
    derivation = (state.spec.get("provenance") or {}).get("derivation")
    assert isinstance(derivation, dict) and derivation.get("expression"), derivation
    buckets = plant.build_order(world)
    assert state.id in {s.id for s in buckets["ready"]}, [s.id for s in buckets["rule"]][:5]
    # And the claim the bucket makes: the plant really does compute it, in one tick, from those
    # paths — 360 W, the sum `check_thermal_heat_inputs` also holds against the zone's loads.
    values = plant.step(world, plant.initial_values(world), 0.02, [])
    assert values["avionics_bay_heat_w"] == 360.0, values.get("avionics_bay_heat_w")

    # The negative, in a broken copy: no derivation, no ready — the state owes the code after all.
    definition = copy_definition(fixture_dir(tmp_path, "no-derivation"))
    path = definition / "domains" / "thermal" / "components.yaml"
    text = path.read_text()
    start = text.index("  - id: avionics_bay_heat_w\n")
    end = text.index("  - id: zone_csm_cabin_t\n")
    block = text[start:end]
    lines = block.split("\n")
    first = next(i for i, line in enumerate(lines) if line == "      derivation:")
    after = next(i for i in range(first + 1, len(lines)) if lines[i].strip() and not lines[i].startswith("        "))
    stripped = "\n".join(lines[:first] + lines[after:])
    assert stripped != block, "the fixture no longer matches the heat rate's derivation"
    path.write_text(text[:start] + stripped + text[end:])
    broken = plant.build_order(plant.load_world(definition))
    assert "avionics_bay_heat_w" in {s.id for s in broken["rule"]}, "a state with no rule was called ready"
...
def test_every_heated_zone_declares_the_state_that_carries_its_heat(tmp_path):
    ...
    The avionics bay was the one that was owed — `heat_inputs.csm_avionics_bay` assigns the IMU, the
    guidance computer and the instrumentation, and no state summed them — and the debt is
    **closed**: `avionics_bay_heat_w` carries the 360 W, and the fixtures below are what keep the
    rule honest now that the corpus no longer trips it. An absent link is still a debt (the checker
    is exercised by a fixture that removes one), a link that does not resolve is still a refusal,
    and two zones still may not share a state.
    """
    thermal = yaml.safe_load((VEHICLE / "domains" / "thermal" / "components.yaml").read_text())
    vehicle = yaml.safe_load((VEHICLE / "vehicle.yaml").read_text())
    power = yaml.safe_load((VEHICLE / "domains" / "power" / "components.yaml").read_text())
    ...
    # Every heated zone names a loop **and** a state that carries exactly its own loads — including
    # the avionics bay, which was the debt this check reported until the round that landed
    # `avionics_bay_heat_w` closed it. The fixture below is what keeps the rule itself honest: the
    # debt is gone from the corpus, not from the check.
    for zone, block in thermal["heat_inputs"].items():
        row = zones[zone]
        assert row["cooled_by"] in loops, zone
        assert "heat_state" in row, zone
        declared = sum(int(loads[i]["demand_w"] or 0) for i in block["loads"])
        assert states[row["heat_state"]]["total_w"] == declared, zone
    assert zones["csm_avionics_bay"]["heat_state"] == "avionics_bay_heat_w"
    ...
    # An absent link is a debt rather than a refusal: the zone's heat has no carrier, and the
    # message names the zone's own figure so the reader can paste it into the state to build.
    absent = fixture("no-state", "        heat_state: service_bay_heat_w\n", "")
    assert absent.returncode == 0, absent.stdout[-900:]
    assert "names no heat-rate state" in absent.stdout, absent.stdout[-900:]
    assert "summing to 630 W" in absent.stdout
...
def test_the_internal_sentinel_is_not_exempt_from_the_ordering_rule():
    ...
    **Round 19 declared all nine**, so this test no longer measures a debt: it holds the nine
    ...
    # 9 -> 10 with eclss, for the same reason as the test above.
...
def test_the_plant_counts_a_regime_table_as_a_declared_sensitivity():
    ...
    assert len(scalar) + len(tables) == 69
    ...
    assert (int(found.group(1)), int(found.group(2))) == (69, len(edges)), found.group(0)
...
def test_the_linter_refuses_a_front_table_that_states_the_wrong_figures(tmp_path):
    ...
    for name, old, new, needle in (
        ...
        ("order", "the 58-node tick order", "the 39-node tick order", "while the linter derives 58"),
    ):
...
def test_the_linter_refuses_a_domain_paragraph_with_no_count_clause(tmp_path):
    ...
    path = definition / "README.md"
    text = path.read_text()
    old = "24 states, 17 thresholds, 5 verbs, 11 faults"
...
def test_every_domain_that_uses_the_sentinel_declares_the_order_it_advances_in():
    ...
    **Nothing on the sentinel advances today** — the rules that would move these states are domain
    ...
    # 9 -> 10 in round 32: eclss's oxygen supply tank is a second state on the sentinel, so that
    # domain declares an order now too.
...
def test_the_window_s_channels_are_counted_rather_than_described(tmp_path):
    ...
    **And this docstring carried the stale copy, which is round 61's finding arriving here.** It said
    ...
    assert (
        "states that 15 of the derived channels carry an evaluable `derivation`, and the registry "
...
def test_every_integrator_declares_where_its_value_starts(tmp_path):
    ...
    # The panel's start moved in round 45: it was the radiator model's own 285 K linearisation
    # point, which is the *design* condition with no absorbed load, and it is the equilibrium the
    # panel's law gives for the environment this corpus declares — the temperature at which the
    # rejection equals the load, so the loop closes from the first tick instead of after a warm-up
    # that put the coolant return above its own band.
    assert states["zone_radiator_t"]["initial_source"] == (
        "domains/thermal/components.yaml:state.radiator_eq_t.total_k"
    )
...
def test_a_node_sourced_channel_is_published_in_the_unit_its_channel_declares():
    ...
    # **The supply-tank pressure used to be the omitted one, and round 32 gave it a producer.** Its
    # source is no longer a mass: `eclss.o2_supply_pressure_psi` reads `o2_supply_pressure_psi`, the
    # state the ECS study guide's working pressure sizes, and the frame carries it at 900 psig — inside
    # the channel's own 750-950 band, which the note below holds.
    assert values["eclss.o2_supply_pressure_psi"] == pytest.approx(900.0)
    low, high = channels["eclss.o2_supply_pressure_psi"]["range"]
    assert low <= values["eclss.o2_supply_pressure_psi"] <= high
    # The zone template is still absent: its `from` is a list of four states and the frame's keys are
    # channel ids, which the registry's own placeholder debt is what closes.
...
def test_the_oxygen_supply_pressure_is_a_state_with_a_published_starting_value(tmp_path):
    """A watched channel had a mass for a producer; the ECS guide publishes what belongs there.

    `eclss.o2_supply_pressure_psi` is apollo's 750-950 psi indication — a low-pressure event and a
    >5 psi/min fall, four declared faults perturbing it, and the channel that corroborates a leak by
    falling while the cabin pressure is held. It read `o2_csm`, the shared oxygen *stock*, in
    kilograms: a pressure is not a function of a mass, so round 30 omitted it rather than publishing
    279 kg under a psi name, and round 32 went and found the quantity.

    `csm_ecs_study_guide.pdf` **PDF p. 12** (printed p. 2-1, section II) says what the supply is: "the
    oxygen is stored in **two cryogenic tanks in the service module**, heaters within the tanks change
    the oxygen from a liquid to a gaseous state", and downstream of the S/M shut-off valve "the **high
    pressure (900 psig)** oxygen is supplied in parallel to the surge tank and backup tank … and to the
    S/M supply regulator". So the volume, the phase change and the working pressure are all published,
    and the state is a *pressure* that starts at 900.

    This test holds the two halves of that: the state's figure against the document's own words (the
    citation is in the field, so this is a reader for it), and the channel's value against the band
    apollo's own threshold declares. What is *not* claimed is the dynamics — the state owes its time
    constant and its driver, and the debt says so.
    """
    import sys as _sys
    ...
    eclss = yaml.safe_load((VEHICLE / "domains" / "eclss" / "components.yaml").read_text())
    state = next(s for s in eclss["state"] if s["id"] == "o2_supply_pressure_psi")
    assert state["unit"] == "psi"
    assert state["initial"] == 900
    source = state["initial_provenance"]["source"]
    assert state["initial_provenance"]["basis"] == "historical"
    # **The citation is in the field, with the page.** A `historical` figure without one is a figure
    # nobody can check, which is the whole reason the source law exists.
    assert "csm_ecs_study_guide.pdf" in source and "PDF p. 12" in source
    assert "900 psig" in source
    # And what it owes is declared rather than implied: no time constant, no driver, and a provenance
    # block whose searches are recorded.
    assert state["tau_s"] == "UNCONFIGURED"
    assert state["provenance"]["basis"] == "UNCONFIGURED"
    assert "Searched:" in state["provenance"]["note"]
    # It is on the sentinel, so its order is declared — and the declared order names every sentinel
    # state, which is the rule the linter holds.
    sentinel = [s["id"] for s in eclss["state"] if s.get("node") == "internal"]
    assert sorted(sentinel) == ["o2_supply_pressure_psi", "suit_loop_flow_cfm"], sentinel
    assert sorted(eclss["internal_order"]) == sorted(sentinel)
    assert eclss["internal_order_note"]

    # The channel reads it, the frame publishes it, and the value is inside the channel's own band.
    world = plant.load_world(VEHICLE)
    values = plant.emit_frame(
        world,
        tick=0,
        seq=0,
        boot_id="0" * 32,
    ...
    row = next(
        ...
        if isinstance(rows, list)
        for r in rows
        if isinstance(r, dict) and r.get("id") == "eclss.o2_supply_pressure_psi"
    )
    assert row["range"][0] <= values["eclss.o2_supply_pressure_psi"] <= row["range"][1]

    # And the point row's source is the state rather than the stock — the defect this round closed.
    points = yaml.safe_load((VEHICLE / "domains" / "eclss" / "points.yaml").read_text())
    point = next(p for p in points["points"] if p.get("channel") == "eclss.o2_supply_pressure_psi")
    assert point["from"] == "o2_supply_pressure_psi"

    # A copy whose state is deleted puts the channel back where it was: the linter refuses a `from`
    # that resolves to nothing, which is what makes the state load-bearing rather than decorative.
    broken = copy_definition(fixture_dir(tmp_path, "no-supply-state"))
    path = broken / "domains" / "eclss" / "points.yaml"
    path.write_text(
        path.read_text().replace("    from: o2_supply_pressure_psi\n", "    from: o2_supply_psi\n", 1)
    )
    result = run_linter(broken)
    assert result.returncode == 1
...
def test_a_prose_row_says_whether_it_is_a_chore_or_a_debt(tmp_path):
    ...
    # The row chosen is one that is still *prose*: `thermal.coolant_return_c` was the fixture until
    # round 45 made it arithmetic, and a row that is already evaluable cannot be given an `owed`
    # sentence to contradict.
    both = fixture(
        "owed-and-evaluable",
        "  - channel: thermal.evaporator_rejection_w\n    from: radiator_rejection_w\n"
        '    layer: estimate\n'
        '    derivation: "the evaporator\'s rejection, capped at its rated capacity, with the '
        'water it consumes charged to res.water_cooling_kg"\n',
        "  - channel: thermal.evaporator_rejection_w\n    from: radiator_rejection_w\n"
        "    layer: estimate\n"
        "    derivation:\n      expression: temperature_k - kelvin_offset\n      inputs:\n"
        "        temperature_k: coolant_loop_t\n        kelvin_offset: 273.15\n"
        "    owed: >-\n      a sentence long enough to pass the length rule and short enough to\n"
        "      read, naming what would close this row rather than asserting it is owed\n",
    )
    assert both.returncode == 1
    assert "declares itself `owed` and carries an evaluable `derivation`" in both.stdout

    # 2. A phrase where a sentence belongs. Since round 45 the corpus has **no** owed row to
    # reword — both of them became arithmetic — so the fixture adds one to a prose row rather than
    # replacing one, which is the same declaration the check is about.
    phrase_dir = copy_definition(fixture_dir(tmp_path, "owed-phrase"))
    phrase_path = phrase_dir / "domains" / "thermal" / "points.yaml"
    original = phrase_path.read_text()
    phrased = original.replace(
        "  - channel: thermal.evaporator_rejection_w\n",
        '  - channel: thermal.evaporator_rejection_w\n    owed: "not yet"\n',
        1,
    )
    assert phrased != original, "the fixture no longer matches the evaporator row"
    ...
    assert moved.returncode == 1
    assert (
        "states that 0 of the prose rows declare in their own `owed` field what would close them, "
        "and the registry now has 1" in moved.stdout
    ), moved.stdout[-900:]

    # And the corpus's own: **none**, since round 45. The two that carried an `owed` sentence were
    # the loop's return temperature and its radiator inlet, and each became arithmetic when the
    # figure it was waiting on landed — the inlet in round 44 (the specific heat) and the return in
    # round 45 (the radiator's own law, at last with a surface temperature to be a function of).
...
def test_the_mechanical_conversions_land_on_the_declarations_that_sized_them():
    ...
    lint = run_linter(VEHICLE)
    assert lint.returncode == 0, lint.stdout[-900:]
    assert "15 of the 71 now carry an evaluable `derivation`" in lint.stdout
    entry = next(
        d
    ...
    assert "15 of the 71 now carry an evaluable `derivation`" in entry
    assert "0 of the prose rows declare in their own `owed` field" in entry
